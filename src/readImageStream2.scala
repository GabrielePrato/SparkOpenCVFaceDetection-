...
object DirectKafkaSparkStream {
  def main(args: Array[String]) {
    ...
    val messages = KafkaUtils.createDirectStream[String, Byte](
      ssc,
      LocationStrategies.PreferConsistent,
      ConsumerStrategies.Subscribe[String, Byte](topic, kafkaParams))

    val names = messages.map(_.key)
    val images = messages.map(_.value)
    names.print()
